...
static int io_open (lua_State *L) {
  ...
  char *FullPath = FileExternal_GetFullPath(filename);
  p->f = fopen(FullPath, mode);
  free(FullPath);
...
LUAMOD_API int luaopen_io (lua_State *L) {
  ...
  createstdfile(L, File_In, IO_INPUT, "stdin");
  createstdfile(L, File_Out, IO_OUTPUT, "stdout");
  createstdfile(L, File_Error, NULL, "stderr");
  return 1;
}
